...
class SmartCachingMiddleware(BaseHTTPMiddleware):
    ...
    async def _initialize_cache(self):
        ...
        try:
            ...
            # Initialize cache with circuit breaker protection
    ...
    def _is_streaming_request(self, request: Request) -> bool:
        ...
        # This would need to parse request body to check for stream parameter
        # For now, simple check based on headers or query params
    ...
    async def _generate_cache_key(self, request: Request) -> tuple:
        ...
        try:
            ...
            # Generate cache key using smart cache method
    ...
    async def _extract_response_data(self, response: Response) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # For other response types, we'd need different handling
    ...
    def _reconstruct_response(self, cached_data: Dict[str, Any]) -> Response:
        ...
        try:
            if cached_data['type'] == 'json':
                ...
                # Add original headers (excluding some)
    ...
    def _calculate_ttl(self, request: Request, response_data: Dict[str, Any]) -> int:
        ...
        # Adjust TTL based on response type/content
        try:
            if response_data.get('type') == 'json':
                data = json.loads(response_data.get('data', '{}'))
                
                # Shorter TTL for responses with high randomness (high temperature)
                if isinstance(data, dict):
                    # Check if this looks like an LLM response
                    if 'choices' in data and data['choices']:
                        # Shorter cache for creative/random responses
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        # Add smart cache stats if available
        if self.smart_cache:
            try:
