...
class SmartCachingMiddleware(BaseHTTPMiddleware):
        ...
        try:
            if isinstance(response, JSONResponse):
                return {
                    "type": "json",
                    "data": (
                        response.body.decode() if hasattr(response, "body") else None
                    ),
    ...
    def _reconstruct_response(self, cached_data: Dict[str, Any]) -> Response:
        ...
        try:
            if cached_data["type"] == "json":
                response = JSONResponse(
                    content=(
                        json.loads(cached_data["data"]) if cached_data["data"] else {}
                    ),
        ...
        return JSONResponse(
            content={"error": "Cache reconstruction failed"}, status_code=500
        )
    ...
    async def clear_cache(self):
        ...
        if self.smart_cache:
            try:
                await handle_error_with_circuit_breaker(
                    "cache", self.smart_cache.clear, context={"action": "clear_cache"}
                )
