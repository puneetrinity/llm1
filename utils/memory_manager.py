...
class MemoryManager:
    ...
    def __init__(self, max_memory_mb: int = 8192):
        ...
        logging.info(f"Memory Manager initialized with {max_memory_mb}MB limit")
    ...
    def _calculate_limits(self) -> MemoryLimits:
        ...
        if self.max_memory_mb > system_memory.available / (1024**2) * 0.9:
            ...
            logging.warning(f"Reducing memory limit from {self.max_memory_mb}MB to {adjusted_mb}MB based on system availability")
        ...
        limits = MemoryLimits(
            ...
            semantic_model_mb=min(500, int(total * 0.08)),  # 8% or 500MB max for semantic model
            faiss_index_mb=int(total * 0.05),  # 5% for FAISS indices
            buffer_mb=int(total * 0.12)      # 12% buffer for system and other operations
        )
    ...
    def allocate(self, component: str, amount_mb: int) -> bool:
        ...
        self.usage_tracking[component] = self.usage_tracking.get(component, 0) + amount_mb
        logging.debug(f"Allocated {amount_mb}MB to {component}. Total: {self.usage_tracking[component]}MB")
    ...
    def deallocate(self, component: str, amount_mb: int = None):
        ...
        if amount_mb is None:
            ...
            self.usage_tracking[component] = max(0, self.usage_tracking[component] - amount_mb)
        
        logging.debug(f"Deallocated {amount_mb}MB from {component}. Remaining: {self.usage_tracking[component]}MB")
    ...
    async def _handle_critical_memory(self):
        ...
        if self.usage_tracking:
            largest_component = max(self.usage_tracking.items(), key=lambda x: x[1])
            logging.error(f"Largest memory user: {largest_component[0]} ({largest_component[1]}MB)")
    ...
    def get_recommendations(self) -> list:
        ...
        if usage['system_used_percent'] > 85:
            recommendations.append("System memory usage is high. Consider reducing max_memory_mb setting.")
        
        # Check component usage
        for component, mb in usage['tracked_components'].items():
            limit = self._get_component_limit(component)
            if mb > limit * 0.8:  # Over 80% of component limit
                recommendations.append(f"Component '{component}' using {mb}MB of {limit}MB limit. Consider optimization.")
        
        # Check total utilization
        if usage['utilization_percent'] > 80:
            recommendations.append("Overall memory utilization is high. Consider enabling more aggressive cleanup.")
