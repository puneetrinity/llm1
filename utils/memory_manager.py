...
class MemoryManager:
    ...
    def _get_allocation_summary(self) -> str:
        ...
        l = self.limits
        return (
            f"Total: {l.total_mb}MB, Models: {l.model_mb}MB, "
    ...
    def check_allocation(self, component: str, requested_mb: int) -> bool:
        ...
        if component_current + requested_mb > component_limit:
            logging.warning(
                f"Component {component} would exceed limit: "
        ...
        if total_new_usage > self.limits.total_mb:
            logging.warning(
                f"Total memory would exceed limit: "
    ...
    def allocate(self, component: str, amount_mb: int) -> bool:
        ...
        self.usage_tracking[component] = (
            self.usage_tracking.get(component, 0) + amount_mb
        )
        logging.debug(
        ...
        while True:
            try:
                ...
                if utilization > self.critical_threshold and self.alerts_enabled:
                    logging.error(
                        f"CRITICAL: Memory usage at {utilization:.1%} "
                    ...
                    logging.warning(
                        f"WARNING: Memory usage at {utilization:.1%} "
                        f"({usage['tracked_total_mb']}MB / {self.limits.total_mb}MB)"
                    )
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health": {
                "status": (
                    "critical"
                    if usage["utilization_percent"] > 95
                    else "warning" if usage["utilization_percent"] > 80 else "healthy"
                ),
