...
class EnhancedDashboard:
    ...
    async def get_comprehensive_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_suggestions = self._get_optimization_suggestions(performance_data, basic_metrics)
            ...
            dashboard = {
                ...
                "cost_optimization": {
                    ...
                    "cache_savings_percent": (cache_stats["hit_rate"] * 30),  # Estimate 30% savings per cache hit
    ...
    def _get_optimization_suggestions(self, performance: Dict[str, Any], metrics: Dict[str, Any]) -> List[str]:
        ...
        if performance["response_time_p95"] > 5:
            suggestions.append("Consider enabling more aggressive caching to reduce response times")
        
        if performance["error_rate"] > 2:
            suggestions.append("High error rate detected - check model availability and warmup settings")
        ...
        if model_costs:
            ...
            if most_expensive[1]["percentage"] > 60:
                suggestions.append(f"Consider routing more queries away from {most_expensive[0]} to reduce costs")
        
        # Resource suggestions
        cache_stats = self.cache.get_stats() if hasattr(self.cache, 'get_stats') else {}
        if cache_stats.get("hit_rate", 0) < 0.3:
            suggestions.append("Low cache hit rate - consider tuning semantic similarity threshold")
        
        # Warmup suggestions
        warmup_stats = self.warmup.get_warmup_stats()
        if any(time > 600 for time in warmup_stats.get("time_since_last_warmup", {}).values()):
            suggestions.append("Some models haven't been warmed up recently - consider reducing warmup interval")
    ...
    def _calculate_cache_savings(self, cache_stats: Dict[str, Any], metrics: Dict[str, Any]) -> float:
        ...
        estimated_savings = total_cost * hit_rate * 0.3  # 30% cost reduction per cache hit
    ...
    def _calculate_optimization_score(self, metrics: Dict[str, Any], performance: Dict[str, Any]) -> float:
        ...
        cost_per_request = metrics["cost_analysis"]["total_estimated_cost"] / max(1, metrics["overview"]["total_requests"])
    ...
    def _get_next_action(self, suggestions: List[str]) -> str:
        ...
        return suggestions[0]  # Return first suggestion if no high priority found
