...
class PerformanceMonitor:
    ...
    async def check_performance_thresholds(self):
        ...
        for alert in alerts_triggered:
            logging.warning(f"Performance alert: {alert['type']} = {alert['value']:.2f} (threshold: {alert['threshold']:.2f})")
    ...
    async def get_current_performance_summary(self) -> Dict[str, Any]:
        ...
        for model, stats in self.model_performance.items():
            if stats["response_times"]:
                ...
                avg_throughput = sum(stats["throughput"]) / len(stats["throughput"]) if stats["throughput"] else 0
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        for model, stats in self.model_performance.items():
            if stats["response_times"]:
                model_avg_times[model] = sum(stats["response_times"]) / len(stats["response_times"])
