...
class OptimizedConnectionPool:
        ...
        if self.connector:
            connector_stats = {
                ...
                "available_connections": sum(
                    len(conns) for conns in self.connector._conns.values()
                ),
                "acquired_connections": self.connector._acquired_per_host,
                "dns_cache_size": (
                    len(self.connector._dns_cache)
                    if hasattr(self.connector, "_dns_cache")
                    else 0
                ),
            }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        total_requests = (
            self.stats["connections_created"] + self.stats["connections_reused"]
        )
        ...
        return {
            ...
            "metrics": {
                ...
                "avg_connections_per_host": stats["connector_stats"].get(
                    "total_connections", 0
                )
                / max(1, len(self.connector._conns)),
            },
