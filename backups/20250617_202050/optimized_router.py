...
try:
    ...
    logging.warning("SemanticIntentClassifier not available - using rule-based classification only")
...
class EnhancedLLMRouter:
    ...
    def __init__(self, ollama_client=None):
        ...
        if BaseRouter:
            ...
            logging.error("Base router not available - enhanced router will have limited functionality")
            ...
            if SemanticIntentClassifier:
                try:
                    ...
                    if self.semantic_classifier.model:
                        ...
                        logging.warning("⚠️ Semantic classifier model not loaded - using rule-based only")
                        self.semantic_classifier = None
                except Exception as e:
                    logging.warning(f"⚠️ Semantic classifier initialization failed: {str(e)}")
                    self.semantic_classifier = None
            else:
                logging.info("ℹ️ Semantic classifier not available - using rule-based classification")
    ...
    async def route_request(self, request) -> str:
        ...
        logging.info(f"🎯 Enhanced routing: intent='{intent}' → model='{selected_model}'")
        return selected_model
    
    async def classify_intent(self, text: str, explicit_intent: Optional[str] = None) -> str:
    ...
    async def _semantic_classification(self, text: str) -> str:
        ...
        try:
            ...
            if confidence >= threshold:
                ...
                logging.debug(f"Semantic classification confidence too low: {confidence:.3f} < {threshold}")
                return 'unknown'
    ...
    def _heuristic_classification(self, text: str) -> str:
        ...
        if word_count < 5:
            ...
            if any(word in text_lower for word in ['analyze', 'review', 'evaluate', 'compare']):
                return 'analysis'
            else:
        ...
        content_indicators = {
            'resume': ['resume', 'cv', 'experience', 'qualifications', 'work history', 'skills'],
            'interview': ['interview', 'job application', 'career', 'hiring', 'position'],
            'creative': ['write', 'create', 'compose', 'story', 'poem', 'article', 'blog'],
            'coding': ['code', 'function', 'algorithm', 'programming', 'script', 'debug'],
            'math': ['calculate', 'solve', 'equation', 'formula', 'math', 'arithmetic'],
            'analysis': ['analyze', 'compare', 'evaluate', 'review', 'assess', 'examine']
        }
    ...
    def _select_model_for_intent(self, intent: str, text: str, request) -> str:
        ...
        for model_name, capabilities in self.model_capabilities.items():
            if model_name in self.available_models and (intent in capabilities or 'general' in capabilities):
        ...
        if not suitable_models:
            suitable_models = [(name, config.get('priority', 99)) 
    ...
    def _get_default_model(self) -> str:
        ...
        if self.available_models:
            ...
            return min(self.available_models.items(), key=lambda x: x[1].get('priority', 99))[0]
        else:
    ...
    def get_routing_recommendations(self) -> List[str]:
        ...
        if total_calls > 0:
            ...
            if semantic_ratio < 0.1:
                recommendations.append("Consider enabling semantic classification for better routing accuracy")
        
        cache_hit_rate = self.cache_stats['hits'] / max(1, self.cache_stats['hits'] + self.cache_stats['misses'])
        if cache_hit_rate < 0.5:
            recommendations.append("Low cache hit rate - consider increasing cache size or review query patterns")
        
        if len(self.available_models) < 2:
            recommendations.append("Consider adding more models for better intent-based routing")
