...
class EnhancedSettings(BaseSettings):
    ...
    OLLAMA_TIMEOUT: int = Field(
        default=300, description="Ollama timeout in seconds")

    # Basic model settings
    DEFAULT_MODEL: str = Field(
        default="mistral:7b-instruct-q4_0", description="Default model")
    ...
    ENABLE_SEMANTIC_CLASSIFICATION: bool = Field(
        default=True, description="Enable semantic classification")
    SEMANTIC_MODEL: str = Field(
        default="all-MiniLM-L6-v2", description="Semantic model name")
    SEMANTIC_CONFIDENCE_THRESHOLD: float = Field(
        default=0.7, description="Semantic confidence threshold")
    CLASSIFICATION_CACHE_SIZE: int = Field(
        default=1000, description="Classification cache size")

    # Streaming Settings
    ENABLE_STREAMING: bool = Field(
        default=True, description="Enable streaming")
    STREAM_CHUNK_SIZE: int = Field(
        default=1024, description="Stream chunk size")
    STREAM_TIMEOUT: int = Field(
        default=300, description="Stream timeout in seconds")

    # Model Warmup Settings
    ENABLE_MODEL_WARMUP: bool = Field(
        default=True, description="Enable model warmup")
    WARMUP_INTERVAL_MINUTES: int = Field(
        default=5, description="Warmup interval in minutes")
    WARMUP_TIMEOUT_SECONDS: int = Field(
        default=30, description="Warmup timeout in seconds")
    ...
    MAX_CONCURRENT_MODELS: int = Field(
        default=3, description="Max concurrent models")
    MODEL_IDLE_TIMEOUT_MINUTES: int = Field(
        default=30, description="Model idle timeout in minutes")
    ENABLE_DYNAMIC_MODEL_LOADING: bool = Field(
        default=True, description="Enable dynamic model loading")

    # Enhanced Caching
    ENABLE_SEMANTIC_CACHE: bool = Field(
        default=True, description="Enable semantic cache")
    SEMANTIC_CACHE_TTL: int = Field(
        default=7200, description="Semantic cache TTL in seconds")
    CACHE_COMPRESSION: bool = Field(
        default=True, description="Enable cache compression")

    # Performance Monitoring
    ENABLE_DETAILED_METRICS: bool = Field(
        default=True, description="Enable detailed metrics")
...
# Dashboard Settings (Required for enhanced features)
    ENABLE_DASHBOARD: bool = Field(
        default=True, description="Enable dashboard")
    ENABLE_WEBSOCKET_DASHBOARD: bool = Field(
        default=True, description="Enable WebSocket dashboard")
    DASHBOARD_UPDATE_INTERVAL: int = Field(
        default=10, description="Dashboard update interval")
    DASHBOARD_PATH: str = Field(
        default="/dashboard", description="Dashboard path")
