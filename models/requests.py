...
class ChatCompletionRequest(BaseModel):
    ...
    intent: Optional[str] = Field(
        None, description="Explicit intent for routing")
    priority: Optional[str] = Field(
        "normal", description="Request priority (low, normal, high)")
...
class CompletionRequest(BaseModel):
    ...
    echo: Optional[bool] = Field(
        False, description="Echo the prompt in the response")
    user: Optional[str] = Field(None, description="User identifier")
...
class ModelRequest(BaseModel):
    ...
    action: Optional[str] = Field(
        "load", description="Action to perform (load, unload, info)")


class WarmupRequest(BaseModel):
    model: str = Field(..., description="Model to warmup")
    force: Optional[bool] = Field(
        False, description="Force warmup even if recently used")
