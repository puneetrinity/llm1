...
class AuthService:
    ...
    def create_session(self, user_id: str, expires_hours: int = None) -> str:
        ...
        logger.info(
            f"🎫 Session created for user: {user_id} (expires: {expires_at})")
    ...
    def _cleanup_user_sessions(self, user_id: str):
        ...
        if len(user_sessions) > max_sessions:
            logger.info(
                f"🧹 Cleaned up {len(user_sessions) - max_sessions} old sessions for {user_id}")
    ...
    def _log_failed_attempt(self, api_key: str):
        ...
        key_hash = hashlib.sha256(
            api_key.encode() if api_key else b"").hexdigest()[:8]
