                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            logger.info(f"✅ Ollama is ready! Found {len(data.get('models', []))} models")
                            self.initialization_state["ollama_connected"] = True
                            self.initialization_state["timestamp"] = datetime.now().isoformat()
                            return True
                            
            except aiohttp.ClientError as e:
                logger.debug(f"Ollama not ready yet (attempt {retry_count + 1}/{self.max_retries}): {str(e)}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.ollama_base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        available_required = [m for m in required_models if m in model_names]
                        missing_models = [m for m in required_models if m not in model_names]
                        
                        self.initialization_state["models_available"] = len(available_required) > 0
