...
class EnhancedLLMRouter:
    ...
    async def route_request(self, request) -> str:
        ...
        # Classify intent
        intent, confidence = await self.classify_intent(
            text_content, getattr(request, "intent", None)
        )
    ...
    async def classify_intent(
        ...
        if self.semantic_classifier:
            try:
                intent, confidence = await self.semantic_classifier.classify_intent(
                    text
                )
    ...
    def _heuristic_classification(self, text: str) -> str:
        ...
        if word_count < 10:
            ...
            return (
                "analysis"
                if any(
                    word in text_lower for word in ["analyze", "evaluate", "compare"]
                )
                else "creative"
            )
    ...
    def _select_model_for_intent(
        self, intent: str, confidence: float, text: str, request
    ) -> str:
    ...
    def _update_routing_stats(self, intent: str, model: str):
        ...
        self.routing_stats["intent_counts"][intent] = (
            self.routing_stats["intent_counts"].get(intent, 0) + 1
        )
        self.routing_stats["model_usage"][model] = (
            self.routing_stats["model_usage"].get(model, 0) + 1
        )
    ...
    def get_classification_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "routing_stats": {
                ...
                "cache_hit_rate": (
                    self.routing_stats["cache_hits"] / max(1, total_requests)
                )
                * 100,
    ...
    def get_routing_recommendations(self) -> List[str]:
        ...
        if intent_counts:
            ...
            intent_percentage = (
                intent_counts[most_common_intent] / total_requests
            ) * 100
