...
class EnhancedLLMRouter:
    ...
    async def _detect_available_models(self):
        ...
        try:
            if self.ollama_client:
                ...
                if not self.available_models:
                    logging.warning("No configured models found, using fallback configuration")
                ...
                logging.warning("No Ollama client available, using default model configuration")
        ...
        for intent, model in list(self.intent_model_mapping.items()):
            if model not in self.available_models:
                ...
                if alternatives:
                    ...
                    logging.info(f"Mapped {intent} to alternative model: {alternatives[0]}")
                elif self.available_models:
                    # Use first available model as fallback
                    fallback = list(self.available_models.keys())[0]
                    self.intent_model_mapping[intent] = fallback
                    logging.info(f"Mapped {intent} to fallback model: {fallback}")
    ...
    async def route_request(self, request) -> str:
        ...
        selected_model = self._select_model_for_intent(intent, confidence, text_content, request)
        
        # Log routing decision
        logging.info(f"🧠 Semantic routing: '{text_content[:50]}...' → intent={intent} (conf={confidence:.2f}) → {selected_model}")
    ...
    def _update_routing_stats(self, intent: str, model: str):
        ...
        self.routing_stats['intent_counts'][intent] = self.routing_stats['intent_counts'].get(intent, 0) + 1
        self.routing_stats['model_usage'][model] = self.routing_stats['model_usage'].get(model, 0) + 1
    ...
    def get_routing_recommendations(self) -> List[str]:
        ...
        if intent_counts:
            ...
            intent_percentage = (intent_counts[most_common_intent] / total_requests) * 100
            
            if intent_percentage > 50:
                recommendations.append(f"Consider optimizing for '{most_common_intent}' intent ({intent_percentage:.1f}% of requests)")
        
        # Analyze model usage
        model_usage = self.routing_stats['model_usage']
        if model_usage:
            most_used_model = max(model_usage, key=model_usage.get)
            usage_percentage = (model_usage[most_used_model] / total_requests) * 100
            
            if usage_percentage > 70:
                recommendations.append(f"Consider keeping '{most_used_model}' warm ({usage_percentage:.1f}% usage)")
        
        # Cache efficiency
        cache_hit_rate = (self.routing_stats['cache_hits'] / max(1, total_requests)) * 100
        if cache_hit_rate < 30:
            recommendations.append(f"Low cache hit rate ({cache_hit_rate:.1f}%) - consider increasing cache size")
    ...
    async def process_chat_completion(self, request, model: str):
        ...
        if self.base_router and hasattr(self.base_router, 'process_chat_completion'):
            ...
            raise NotImplementedError("Base router required for processing chat completions")
